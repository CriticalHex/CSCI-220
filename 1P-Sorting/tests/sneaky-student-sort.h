...
void set_student_sort_failure(const char *const n) {
  /***
   * No, this output is not buggy.  Turns out on at least one common CPP library
   * distribution all four sorting routines lead to standard sort.
   */
  std::cerr << std::endl;
  std::cerr
      << "Please don't use std::" << n
      << " or it's variants to satisfy this project's sorting requirements."
      << std::endl;
  std::cerr << "Implement merge sort as specified in the project write-up."
            << std::endl
            << std::endl;
 ...
 * we don't need to provide constexpr (return value) specializations for sort
 * because since we invoke it from the SneakyStudentSort TEST we know the
 * arguments on not themselves constexpr.
 */

#define SNORT(ce, funcname)                                                    \
  template <>                                                                  \
...
SN_CONSTEXPR vector<student_sort_object>::iterator
partial_sort_copy(InputIt, vector<student_sort_object>::iterator r,
                  vector<student_sort_object>::iterator,
                  vector<student_sort_object>::iterator,
                  vector<student_sort_object>::iterator) {
...
// since C++ 17
SNORTExc(sort) SNORTExcCmp(sort)

    template <class ExcPolicy>
    SN_CONSTEXPR
    void partial_sort(ExcPolicy &&, vector<student_sort_object>::iterator,
                      vector<student_sort_object>::iterator,
                      vector<student_sort_object>::iterator) {
  set_student_sort_failure("partial_sort");
}
...
template <class ExcPolicy, class Compare>
SN_CONSTEXPR void partial_sort(ExcPolicy &&,
                               vector<student_sort_object>::iterator,
                               vector<student_sort_object>::iterator,
                               vector<student_sort_object>::iterator, Compare) {
  set_student_sort_failure("partial_sort");
...
vector<student_sort_object>::iterator
partial_sort_copy(ExcPolicy &&policy, ForwardIt first, ForwardIt last,
                  vector<student_sort_object>::iterator r,
                  vector<student_sort_object>::iterator) {
  set_student_sort_failure("partial_copy_sort");
  return r;
}
...
SNORTCmp(SN_CONSTEXPR, stable_sort)
    ...
    // since C++ 17
    SNORTExc(stable_sort) SNORTExcCmp(stable_sort)
...
TEST(SorterTest, SneakyStudentSort) {
  ...
  // reset flag, its a global variable that students code could just as easily
  // reach out and modify, so we need to not only be sure it remains true but
  // also that the out of order vector is in fact now ordered.
